...
enum enChoices { Show = 1, Add, Delete, Update, Find, Exit };
...
void ShowClients()
{
	...
	else
	{
		for (stClient& c : vClients)
		{
		  cout  << "| " << left << setw(11) << c.AccountNum
				<< "| " << left << setw(8) << c.PinCode
				<< "| " << left << setw(30) << c.Name
				<< "| " << left << setw(10) << c.Phone
				<< "| " << left << setw(30) << c.AccountBalance << endl;
		}
...
void PerformMainMenueOption(enChoices choice)
{
	system("cls");
	switch (choice)
	{
